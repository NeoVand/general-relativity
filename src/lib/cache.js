...
async function prune(db){return new Promise(resolve=>{const tx=db.transaction('cache','readwrite'),s=tx.objectStore('cache'),r=s.getAll();r.onsuccess=()=>{const entries=r.result.sort((a,b)=>b.time-a.time);let bytes=0;entries.forEach((e,i)=>{bytes+=e.value instanceof Blob?e.value.size:JSON.stringify(e.value).length*2;if(i>300||bytes>80*1024*1024)s.delete(e.key)})};tx.oncomplete=resolve;tx.onerror=resolve})}
