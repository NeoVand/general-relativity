<script>
 ...
 import {readingPosition,restoreReadingPosition,focusReadingTarget} from './lib/reading-position.js';
 let {initial}=$props();
 let page=$state(untrack(()=>initial));
 let navigationError=$state('');
 let cleanup=()=>{};
 let navigating=false;
 function rememberPosition(){if(!navigating)history.replaceState({...history.state,reading:readingPosition()},'');}
 ...
 async function navigate(target,{pop=false,highlight=false,signal,position=null}={}){
  ...
  if(!pop)rememberPosition();
  navigating=true;document.body.dataset.readerNavigating='true';
  ...
  if(pop&&position){await document.fonts.ready;restoreReadingPosition(position,revealLocation);}
  else focusReadingTarget(url.hash?document.getElementById(hashId(url.hash)):document.querySelector('#main h1'));
  }finally{if(token===routeToken){navigating=false;document.body.dataset.readerNavigating='false';rememberPosition();}}
 }
 onMount(()=>{
  let alive=true,scrollFrame=0;
  const previousRestoration=history.scrollRestoration;history.scrollRestoration='manual';
  const initialPosition=history.state?.reading;
  const remember=()=>{cancelAnimationFrame(scrollFrame);scrollFrame=requestAnimationFrame(rememberPosition)};
  window.addEventListener('scroll',remember,{passive:true});
  window.addEventListener('pagehide',rememberPosition);
  activate().then(async()=>{await document.fonts.ready;if(alive&&routeToken===0){restoreSnapshot(new URL(location.href));if(initialPosition)restoreReadingPosition(initialPosition,revealLocation);else if(location.hash)revealLocation(document.getElementById(hashId(location.hash)))?.scrollIntoView({block:'start'});}}).catch(()=>{if(alive)navigationError='The interactive reader could not start. Reload to try again.'});
  const click=e=>{
  ...
  const pop=event=>{cancelAnimationFrame(scrollFrame);navigate(location.href,{pop:true,position:event.state?.reading}).catch(e=>navigationError=e.message)};
  document.addEventListener('click',click);window.addEventListener('popstate',pop);
  return ()=>{alive=false;routeToken++;cleanup();cancelAnimationFrame(scrollFrame);history.scrollRestoration=previousRestoration;window.removeEventListener('scroll',remember);window.removeEventListener('pagehide',rememberPosition);document.removeEventListener('click',click);window.removeEventListener('popstate',pop)};
 });
</script>
{#key page.id}
