<script>
 ...
 let history=loadHistory(),wordHighlighter,wordElement,wordFrame=0,lastHistoryWrite=0,packet,hovered,readingMode='flow',voiceTurnController,voiceTurn=0,contextGeneration=0;
 ...
 function handToNarrator(){
  ...
  channel?.send({type:'response.cancel'});channel?.send({type:'output_audio_buffer.clear'});channel?.send({type:'input_audio_buffer.clear'});
 }
 ...
 async function runTool(name,args,signal){
  ...
  if(name==='control_narration'){
   ...
   if(args.action==='resume')await resume();else if(args.action==='pause')pause();else stop();
 ...
 async function ask(text=question){
  ...
  pause();voiceTurn++;voiceTurnController?.abort();if(calling){channel?.send({type:'response.cancel'});channel?.send({type:'output_audio_buffer.clear'});callAudio.muted=true;microphone?.getAudioTracks().forEach(t=>t.enabled=false);narratorOwnsAudio=true;}question='';open=true;tab='ask';error='';thinking=true;
  ...
  try{
   ...
   for(let round=0;round<7;round++){
    ...
    if(result.tool_calls?.length){
     let handedOff=false;
     for(const call of result.tool_calls){let args;try{args=JSON.parse(call.function.arguments)}catch{args={}}const output=await toolResult(call.function.name,args,signal);if(token!==chatGeneration)return;input.push({role:'tool',tool_call_id:call.id,content:JSON.stringify(output)});if(output.playback==='started')handedOff=true;}
 ...
 async function startCall(){
  ...
  try{
   ...
   channel.send({type:'session.update',session:{type:'realtime',audio:{input:{transcription:{model:'gpt-4o-mini-transcribe'},turn_detection:{type:'server_vad',create_response:false,interrupt_response:true}}}}});
 ...
 async function handleEvent(event,generation){
  if(generation!==callGeneration)return;
  if(event.type==='input_audio_buffer.speech_started'){if(narratorOwnsAudio)return;pause();voiceTurn++;voiceTurnController?.abort();callState='listening';caption='';}
  if(event.type==='conversation.item.input_audio_transcription.completed'&&event.transcript){if(narratorOwnsAudio)return;messages.push({id:event.item_id||crypto.randomUUID(),role:'user',text:event.transcript});const turn=voiceTurn;await syncVoiceContext();if(generation===callGeneration&&turn===voiceTurn&&!narratorOwnsAudio)channel?.send({type:'response.create'});}
  if(event.type==='response.output_audio_transcript.delta'||event.type==='response.output_text.delta'){
   ...
   if(!entry){messages.push({id,role:'assistant',text:'',model:'GPT Realtime 2.1'});entry=messages.at(-1)}entry.text+=event.delta||'';caption=entry.text;callState='speaking';
  ...
  if(event.type==='response.done'){
   ...
   const turn=voiceTurn;voiceTurnController=new AbortController();let handedOff=false;
   for(const call of calls){seenCalls.add(call.call_id);let args;try{args=JSON.parse(call.arguments)}catch{args={}}const result=await toolResult(call.name,args,voiceTurnController.signal);if(generation!==callGeneration||turn!==voiceTurn)return;if(result.playback==='started')handedOff=true;channel?.send({type:'conversation.item.create',item:{type:'function_call_output',call_id:call.call_id,output:JSON.stringify(result)}});}
   if(!handedOff)channel?.send({type:'response.create'});
  }
...
{#if active}
 <div class="listening-dock">
  {#if liveTokens.length}<div class="listening-caption" aria-label="Spoken words"><span class="caption-location">{readingHeading}</span><p>{#each liveTokens as word (word.index)}<span class:spoken-current={word.index===spokenWord}>{word.text}</span>{/each}</p></div>{/if}
 <div class="listening-bar" aria-label="Narration player">
  ...
  <button class="player-title" onclick={()=>reveal('listen')}><span>{busy?'Preparing your reading…':readingTitle}</span><small>Passage {readingIndex+1} of {readingCount} · ElevenLabs</small></button>
  <button aria-label="Ask about this reading" onclick={talkAboutReading}><Icon name="mic"/></button>
