/*! Adapted from Voicebook. Copyright (c) 2026 NeoVand. MIT license: assets/licenses/voicebook.txt */
/**
...
function errorMessage(status: number, data: unknown, model: string): string {
  ...
  if (status === 401 || status === 403) {
    return 'The OpenAI API key was rejected. Check it under Connections.';
  }
  if (status === 404) {
...
function waitForChannel(channel: RTCDataChannel, signal?: AbortSignal): Promise<void> {
  if (signal?.aborted) return Promise.reject(new RealtimeError('The request was cancelled.'));
