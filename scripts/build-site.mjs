...
const clientAssets={};
for(const name of ['styles.css','course.css','visual-lessons.css','course.js','visual-lessons.js','symbols.js','app.js','earth-flow.js','scenes.js']){
 const source=Buffer.from(fs.readFileSync(`web/${name}`,'utf8').replace("'./earth-flow.js'",`'./${clientAssets['earth-flow.js']||'earth-flow.js'}'`).replace("'./symbols.js'",`'./${clientAssets['symbols.js']||'symbols.js'}'`)),hash=createHash('sha256').update(source).digest('hex').slice(0,12);
...
for(let i=0;i<pages.length;i++){
 ...
 for(const f of figures){
  if(f.after&&!paired.has(f.id)&&!references.has(f.id)){const anchor=new RegExp(`(<h3[^>]*>${f.after.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}[\\s\\S]*?<\\/h3>)`);html=html.replace(anchor,`$1${figure(f)}`);}
 }
 ...
 const opening=figures.filter(f=>!f.after&&!paired.has(f.id)&&!references.has(f.id)).map(figure).join('');
