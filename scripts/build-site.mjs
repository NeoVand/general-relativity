...
const clientAssets={};
for(const name of ['styles.css','symbols.js','app.js','earth-flow.js','scenes.js']){
...
function nav(active,headings=[]){
 ...
 return `<div class="nav-inner"><div class="nav-home"><button id="menu-button" class="icon-button" aria-expanded="false" aria-controls="book-navigation">${icon('SidebarLeftIcon')} <span>Contents</span></button><a class="nav-home-link nav-copy" href="index.html" ${active==='index'?'aria-current="page"':''}>General relativity</a></div><nav class="nav-scroll" aria-label="Book chapters">${groups.map(([,a,b],i)=>group(`part-${i}`,navParts[i][0],navParts[i][1],navParts[i][2],chapterRole(+a),pages.filter(p=>p.chapter!==undefined&&+p.chapter>=+a&&+p.chapter<=+b),a===b?a:`${a}–${b}`)).join('')}${group('notebook','Notebook','Appendices & reference','Notebook01Icon','geometry',pages.filter(p=>p.appendix),'')}</nav><nav class="nav-utilities" aria-label="Book resources">${link('figure-atlas','Visual atlas','CubeIcon')}${link('reading-guide','Reading guide','Compass01Icon')}${link('visual-language','Color & notation','ColorsIcon')}${link('credits','Edition notes','InformationCircleIcon')}</nav></div>`;
}
...
for(let i=0;i<pages.length;i++){
 ...
 const chapterScenes=scenes.filter(s=>String(s.chapter)===p.chapter);
 const paired=new Set(chapterScenes.filter(s=>s.id!=='earth').map(s=>s.figure));
 for(const f of figures){
  if(f.after&&!paired.has(f.id)){const anchor=new RegExp(`(<h3[^>]*>${f.after.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}[\\s\\S]*?<\\/h3>)`);html=html.replace(anchor,`$1${figure(f)}`);}
 }
 const opening=figures.filter(f=>!f.after&&!paired.has(f.id)).map(figure).join('');
