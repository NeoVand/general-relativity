...
const clientAssets={};
for(const name of ['styles.css','app.js','scenes.js']){
 const source=fs.readFileSync(`web/${name}`),hash=createHash('sha256').update(source).digest('hex').slice(0,12);
...
const nav=active=>`<a class="brand" href="index.html"><span class="brand-mark">${icon('Orbit01Icon')}</span><span>GENERAL RELATIVITY<small>From the Inside Out</small></span></a><div class="nav-label">THE READING ROOM</div><a class="nav-special" href="reading-guide.html">How to read · Conventions</a><nav aria-label="Book chapters">${pages.filter(p=>p.chapter!==undefined).map(p=>`<a href="${p.id}.html" ${p.id===active?'aria-current="page"':''}><span class="chapter-index">${String(p.chapter).padStart(2,'0')}</span><span>${esc(short(p))}</span></a>`).join('')}<div class="nav-label">KEEP BESIDE YOUR NOTEBOOK</div>${pages.filter(p=>p.appendix).map(p=>`<a href="${p.id}.html" ${p.id===active?'aria-current="page"':''}><span class="chapter-index">${p.appendix}</span><span>${esc(short(p))}</span></a>`).join('')}</nav><a class="nav-special" href="figure-atlas.html">${icon('CubeIcon')} The visual atlas</a><a class="nav-special" href="visual-language.html">${icon('ColorsIcon')} The language of color</a><a class="nav-special" href="credits.html">Images & edition notes</a><a class="repo-link" href="https://github.com/NeoVand/general-relativity">Source on GitHub ↗</a>`;
function layout(title,body,{active='',rail='',description='A visual, step-by-step general relativity textbook, beginning with basic calculus and linear algebra.'}={}){
 return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="description" content="${esc(description)}"><meta name="theme-color" content="#102b3b"><title>${esc(title)} · General Relativity</title><link rel="icon" href="assets/favicon.svg" type="image/svg+xml"><script>try{document.documentElement.dataset.theme=localStorage.getItem("gr-theme")||(matchMedia("(prefers-color-scheme:dark)").matches?"dark":"light");if(localStorage.getItem("gr-type")==="large")document.documentElement.classList.add("large-type")}catch{}</script><link rel="preload" href="assets/fonts/manrope-latin-wght-normal.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="assets/fonts/newsreader-latin-wght-normal.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="assets/katex/katex.min.css"><link rel="stylesheet" href="${clientAssets['styles.css']}"><script src="assets/katex/katex.min.js" defer></script><script src="${clientAssets['app.js']}" defer></script><script type="importmap">{"imports":{"three":"./assets/three/three.module.js"}}</script>${body.includes('data-scene=')?`<script type="module" src="${clientAssets['scenes.js']}"></script>`:''}</head><body data-page="${active}"><a class="skip-link" href="#main">Skip to reading</a><aside class="sidebar" id="book-navigation">${nav(active)}</aside><div class="reading-progress" aria-hidden="true"></div><div class="page"><header class="topbar"><button id="menu-button" class="icon-button" aria-expanded="false" aria-controls="book-navigation">${icon('Menu01Icon')} <span>Contents</span></button><a href="index.html" class="top-title">A FIELD GUIDE TO SPACETIME</a><div class="reading-tools"><button id="search-button" class="text-button" aria-haspopup="dialog">${icon('Search01Icon')}<span>Search</span></button><button id="type-button" class="text-button" aria-label="Increase reading text size" aria-pressed="false">${icon('TextFontIcon')}</button><button id="theme-button" class="text-button" aria-label="Switch to dark appearance">${icon('Moon02Icon')}</button></div></header><div class="reading-layout"><main id="main">${body}</main>${rail?`<aside class="section-rail" aria-label="On this page">${rail}</aside>`:''}</div><footer class="site-footer"><span>General Relativity, From the Inside Out</span><a href="credits.html">Sources & image credits</a><span>Built to be read, questioned, and understood.</span></footer></div><dialog id="search-dialog" aria-labelledby="search-heading"><div class="search-header"><h2 id="search-heading">Find an idea</h2><button id="search-close" class="text-button">Close</button></div><label for="search-input">Search the book</label><input id="search-input" type="search" placeholder="Try proper time, pressure, or a section number" autocomplete="off"><p id="search-status" role="status"></p><div id="search-results"></div></dialog><dialog id="figure-dialog" aria-labelledby="figure-dialog-title"><div class="search-header"><h2 id="figure-dialog-title">Figure detail</h2><button id="figure-close" class="text-button">${icon('Cancel01Icon')}<span>Close</span></button></div><p class="figure-instruction">Scroll to explore at full resolution. Colors follow your reading theme.</p><div id="figure-detail"></div></dialog></body></html>`;
}
...
for(let i=0;i<pages.length;i++){
 ...
 const guide=g?`<section class="chapter-guide"><div><span class="eyebrow">THE QUESTION</span><p>${inline(g.question,p.chapter)}</p></div><div><span class="eyebrow">BRING WITH YOU</span><p>${inline(g.needs,p.chapter)}</p></div><p class="chapter-payoff"><strong>By the end:</strong> ${inline(g.payoff,p.chapter)}</p></section>`:'';
 const check=g?`<section class="takeaway"><span class="eyebrow">PAUSE BEFORE YOU TURN THE PAGE</span><h2>The idea to keep</h2><p>${inline(g.takeaway,p.chapter)}</p><details class="checkpoint"><summary>${inline(g.check,p.chapter)}</summary><p>${inline(g.answer,p.chapter)}</p></details></section>`:'';
...
const heroMath=typeset(String.raw`\underbrace{R_{\mu\nu}-\tfrac12 Rg_{\mu\nu}}_{\text{curvature and metric}}+\underbrace{\Lambda g_{\mu\nu}}_{\text{vacuum term}}=\frac{8\pi G_N}{c^4}\underbrace{T_{\mu\nu}}_{\text{matter and energy}}`,true,12);
