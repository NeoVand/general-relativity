...
b=axes(85,375,420,255,'time t (seconds)','position x (metres)')
b+=plot(lambda t:3*t*t,0,3,85,375,390,240,0,27)
b+=plot(lambda t:12*t-12,1,3,215,375,260,240,0,27,color=ORANGE,dash='7 5')
b+=circle(345,375-240*4/9,6,TEAL)+text(326,243,'t = 2',17,anchor='end')
b+=line(331,248,340,261,MUTED,1.5)
chart=responsive_group(b,transform='translate(-25 5)')
b=label(565,130,'The cart at two seconds','Position: 12 m. Velocity: 12 m/s.')
b+=line(570,191,920,191)+text(565,234,'Δx ≈ (12 m/s) Δt',26,ORANGE)
b+=text(565,283,'The line matches value and slope.',19)+text(565,316,'The cart continues to speed up.',19)
chart+=responsive_group(text(85,435,'Curve: x = (3 m/s²) t²',18,TEAL)+text(85,465,'Line: x = (12 m/s) t − 12 m',18,ORANGE),transform='translate(-25 5)')
b=chart+responsive_group(b,transform='translate(-505 390)')
for size in (17,18,19):b=b.replace(f'font-size="{size}"','font-size="21"')
save('local-prediction',0,'A derivative predicts the next small step','A parabola and its tangent at t=2. They meet with equal slope but differ away from the contact point.','At two seconds the cart is 12 metres from the start and moving at 12 metres per second. The straight line predicts its next small displacement; the curve also includes its acceleration.',b,mobile_viewbox='0 0 500 750')

# 1: relative motion, not a force-field cartoon.
b=label(55,90,'One test body','An accelerometer reads zero.')+label(545,90,'Two test bodies','Their separation can change.')
...
save('free-fall-comparison',1,'Comparing two free falls','One worldline is compared with a pair of approaching free-fall worldlines, with time increasing upward.','The paired experiment measures relative acceleration. Curved drawn paths are coordinate schematics; their appearance alone is not evidence of curvature.',b)
