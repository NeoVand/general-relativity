...
for(const file of fs.readdirSync('site').filter(f=>f.endsWith('.html'))){
 ...
 for(const el of candidates){
  ...
  entry.narration=narrationNote(el,kind);segments.push(entry);
 ...
 catalog.push({id,title:entry.title,segments:segments.map(({id,index,kind,heading,text,latex,hash})=>({id,index,kind,heading,text,latex,hash}))});
