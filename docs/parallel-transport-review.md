...
## Dark-theme surface repair

The dark theme originally passed its luminous mint text color into a strongly lit surface material. That washed out the surface and its blue route. The shared renderer now uses a separate deep-teal albedo, less ambient light, and a key light directed onto the visible hemisphere. A neutral ground fill keeps the shadowed side readable during orbit. The unlit path, direction arrows and tangent patch retain their semantic colors without filmic whitening. The light theme retains its original surface color and lighting; a pixel comparison of the sphere scene against the pre-change capture found no changed channels.

Both transport experiments and all three surfaces were inspected at 1440 px and 390 px, including the face-on comparison and a view from the opposite side. The SVG fallback uses a corresponding subdued surface palette. Inspection also found that a context loss exposed the original fallback geometry until the next interaction. Recovery now redraws the current state immediately and disables unavailable camera controls; the browser regression checks the selected surface and journey percentage before any further input. Newly created angle markers also receive their theme color immediately.
