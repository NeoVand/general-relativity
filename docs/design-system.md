...
The sidebar adapts the anchored rail and disclosure interactions from the author's Terminalvibes project. A single navigation tree morphs between a 64 px desktop rail and a 292 px panel in 200 ms; icons stay centered at x = 32 px alongside the header toggle. Labels fade, closed descendants become inert, and a noninteractive hover/focus preview identifies the chapters behind each part icon. Clicking that icon or pressing Enter unfolds the real links.
...
At 800 px and below, the panel becomes a drawer above a blurred scrim, without reserving reading width. Escape, the header toggle, outside clicks, and link selection dismiss it. Keyboard focus remains within the drawer and its toggle until dismissal. Existing tutor and playback controls sit behind the drawer, and dismissing navigation preserves an open tutor. Reduced-motion preferences disable the morph. `scripts/check-navigation.mjs`, included in `test:browser`, checks these behaviors and captures both themes at six widths.
