...
The sidebar adapts the anchored rail and disclosure interactions from the author's Terminalvibes project. A single navigation tree morphs between a 64 px desktop rail and a 292 px panel in 200 ms; icons stay centered at x = 32 px under the header’s Black Hole app logo. Labels fade, closed descendants become inert, and a noninteractive hover/focus preview identifies the chapters behind each part icon. Clicking that icon or pressing Enter unfolds the real links.

The five course parts share the homepage's chapter ranges and colors. The current chapter reveals its parent when navigating to a new chapter; explicit group and section disclosure choices otherwise persist. A chapter's chevron exposes its section links, which track the reading position. Appendices live in Notebook. The sidebar has a translucent surface, quiet active fills, and no separator lines.

At 800 px and below, the panel becomes a drawer above a blurred scrim, without reserving reading width. The header’s Black Hole logo opens it on phones; Escape, either toggle, outside clicks, and link selection dismiss it. The sidebar toggle leads the landing-page link in the expanded panel and is the top icon of the collapsed desktop rail. On desktop the header logo links to the landing page. Keyboard focus remains within the drawer and its toggle until dismissal. Existing tutor and playback controls sit behind the drawer, and dismissing navigation preserves an open tutor. Reduced-motion preferences disable the morph. `scripts/check-navigation.mjs`, included in `test:browser`, checks these behaviors and captures both themes at six widths.
