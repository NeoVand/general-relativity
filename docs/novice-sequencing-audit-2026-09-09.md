...
A first-mention check also found the historical use of “metric” in §1.6, the name “geodesic” in §5.2 before its introduction, and Ricci values in §8.6 before the contraction definitions. The historical sentence now states the measuring rule, the test-particle qualification uses already-taught language, and the sphere calculation defines both contractions before evaluating them. The particle action names its integrand as the Lagrangian at first use. This check required reading context: an ordinary pressure “gauge” in Chapter 11 is not an early introduction of gauge symmetry.
