...
- Passage and tool-triggered playback keep your current panel layout. **Minimize** leaves a compact player; its **CC** button toggles floating captions. Clicking the passage title reopens full narration controls.
- Inspect, edit, or regenerate the current spoken explanation. Prepare a chapter's explanations ahead of listening if desired.
- Ask by typing, or explicitly start a microphone conversation. The tutor can search the book, read a passage, and navigate to and highlight an exact source passage. Mute and End call release control of the microphone appropriately; use **Minimize** to keep an active call in a compact dock with visible microphone state, mute, and end-call controls. Reopen it from the dock or Ask.

Hold **⌘ + Shift + Space** on macOS, or **Ctrl + Shift + Space** elsewhere, to ask about the current reading. Holding pauses narration and captures its exact passage, playhead, spoken words, and live lab state. Release to send the question; the microphone is closed between questions. “Continue reading” resumes the paused audio. The first activation connects the voice session; wait for the listening indicator. Releasing while it connects keeps the microphone off. Window blur, tab hiding, and Escape also release it safely. Under **Connections → Hold to ask**, record another combination (including two modifiers alone) or disable the shortcut. Shortcuts apply to this focused browser tab and ignore typing fields.
...
The tutor uses validated `get_book_outline`, `get_reader_focus`, `search_book`, `read_passage`, `show_chapter`, `show_passage`, `clear_highlight`, `play_section`, and `control_narration` tools. The voice tutor also has `consult_text_tutor`, which calls the selected Terra/Sol model for physics explanations. Typed replies label the model that answered; voice consultations show a working status. Realtime manages the conversation while the text model supplies source-grounded explanations. Targets must exist in the local book index. The tutor has no arbitrary URL navigation, shell, network-fetch, or page-script tool. The current chapter, selected or visible passage, and actual slider values ground explanations. Visible labs publish their descriptions, model parameters, readouts, assumptions, available controls, and teaching prompts. Older geometry/curvature labs and spatial scenes that publish a live narration description join this same context. Opening or minimizing the panel preserves the reader’s position. This is structured page awareness, not image vision: unreported camera angles or arbitrary pixels are not visible to the agent. Tool calls cannot access API keys. Model output passes through Markdown with raw HTML disabled, KaTeX, and DOMPurify before display.

## Shared listening and assistant state

Navigation and narration are separate actions. “Take me to chapter 7” opens the chapter using `show_chapter`; `show_passage` reveals an exact section. Neither starts audio. The tool instructions distinguish navigation from listening, and a local guard rejects narration for common explicit navigation-only commands. This guard is deliberately narrow, not a complete natural-language intent classifier.

Voice tools are queued per response and per conversation turn. `response.done` signals the end of generation; when the response contains speech, tools wait for the matching `output_audio_buffer.stopped`. A function-only response needs no playback wait. Clearing audio, cancelled/failed responses, a new user turn, and ending the call invalidate pending work. Response metadata ties late results to their original turn. The completed narration handoff does not send `response.cancel` or clear the assistant's output buffer. Direct player actions can still interrupt speech immediately. These event semantics follow the [Realtime playback event reference](https://developers.openai.com/api/docs/api-reference/realtime-server-events/output_audio_buffer/stopped).

A voice or typed request to read a section calls `play_section`, which starts ElevenLabs at validated start/end anchors. Successful handoff does not request another Realtime response. The microphone is disabled while the narrator has the floor to prevent speaker feedback, but the connection and conversation stay open. Tap the microphone on the player to ask: narration pauses at its current audio position, input reopens, and the next question receives updated source and playhead context. Ask for continuation or press resume to continue that same audio. Finishing or stopping narration restores the connected microphone to its previous mute setting. Ending the call releases microphone tracks.
...
`npm run build` generates figures, the book, the semantic reading index, and the Svelte bundle. `npm run build:reader` rebuilds the book and reader against existing figure assets. `npm run build:app` rebuilds only the Svelte app and replaces its bundle references idempotently; use it after editing `src/`. Rebuild the book after changing its manuscript, metadata, or standalone `web/` modules. `npm run check` runs Svelte diagnostics. Run `npm run check` and `npm run test:all` locally before pushing. The Pages workflow builds and uploads the site without repeating the browser installation and full inspection suite.

The study tests mock the provider boundary and WebRTC peer, while exercising real browser UI, media playback, IndexedDB, chapter navigation, tool handling, and settings. The scheduler tests cover both event orders, silent tool responses, duplicate events, cancelled/failed/incomplete responses, stale turns, and interrupted playback. Browser tests simulate spoken navigation and narration handoffs, reject an unrequested reading, and check compact controls from 320 to 1440 pixels in both themes. They also check request model IDs and payloads, timing and word matching, content-versioned listening history, all section ranges, caching, math rendering, invalid-target rejection, ElevenLabs handoff without overlapping speech, pause/resume at the same audio position, Realtime-to-text-model consultation, microphone teardown, and light/dark desktop/mobile layouts. These tests verify the integration contract and browser behavior; live provider audio quality and account access require the reader's keys.
